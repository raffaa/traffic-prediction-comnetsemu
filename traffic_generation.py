...
def linear_traffic(host1, host2, start_time, current_time, duration):
    ...
    host1.cmd(f"iperf -c {host2.IP()} -b {bandwidth}M -t 0.5 &")
    host1.cmd(f"iperf -u -c {host2.IP()} -b {bandwidth}M -t 0.5 &")  # Adding UDP traffic
...
def constant_traffic(net):
    for host in net.hosts:
        for other_host in net.hosts:
            if other_host != host:
                host.cmd(f"iperf -c {other_host.IP()} -b 1M -t 0.5 &")
                host.cmd(f"iperf -u -c {other_host.IP()} -b 1M -t 0.5 &")  # Adding UDP traffic
